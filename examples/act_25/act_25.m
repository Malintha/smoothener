...
% read input files
discrete_plan_file = 'examples/act_25/act_25_sol.json';
octree_file = 'examples/act_25/act.bt';
...
if (PLOTTING)
    ...
    stl_file = 'examples/act_25/act.stl';
    fv = stlread(stl_file);
    patch(fv, ...
        'FaceColor', [0.4 0.4 0.4], 'EdgeColor', 'none', ...
        'SpecularStrength', 0.1, 'AmbientStrength', 0.5, 'facealpha', 0.2);
    ...
    for i = 1:7
        ellipsoid(starts(i,1),starts(i,2),starts(i,3), 0.25,0.25,0.50);
        ellipsoid(goals(i,1),goals(i,2),goals(i,3), 0.25,0.25,0.50);
    end
    for i = 8:16
        ellipsoid(starts(i,1),starts(i,2),starts(i,3), 0.15,0.15,0.4);
        ellipsoid(goals(i,1),goals(i,2),goals(i,3), 0.15,0.15,0.3);
    end
    for i = 17:25
        ellipsoid(starts(i,1),starts(i,2),starts(i,3), 0.2,0.2,0.4);
        ellipsoid(goals(i,1),goals(i,2),goals(i,3), 0.2,0.2,0.4);
    end
end
...
SAVE_CSVS = true;
if SAVE_CSVS
	DIR = 'examples/act_25/act_25_pps';
	mkdir(DIR);
	for i=1:N
